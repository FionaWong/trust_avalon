...
(function (window,$,account) {
	$(document).ready(function(){
        //common functions
        /*getFloatIcons('body',['wx','fk','db','sc'],'licai');
        setHoverHeader("lc");*/
        productId = common.getParam("productId") || "1003076";

        //取url中的amount塞入Input中
        buyMoneyOperator.setAmount();
        //page render
  ...
  services = function(url_config,productId){
    ...
    var 
        notLoginCode = "900002",
        successCode = "000000",
        productId = productId || common.getParam("productId"),//产品id
        detail_obj,//拿到detail接口数据
        sendAjaxRequest = function(url,data,type){
          return $.ajax({
            type: type || 'GET',
            url:url || "",
            dataType:"jsonp",
    ...
    return  {
      loginState : function(){
          var url = protocol+"/memberAlias.jsp",
            data = {};

          sendAjaxRequest(url,data)
          .done(function(json){
              if(!json.isLoginEmall) { 
                  common.loginPop();//common.alert("请重新登陆");
                  return;
              } else{
                  toInforPage(buyMoney);
              } 
          })
          .fail(function(e){
      ...
      productDetail : function(callbackList){
          ...
          .done(function(json){
              if(json && json.responseCode == '000000'){
                  var left = json.responseData.residueQuota;
                   validateRules(left,val) && detail_obj.isOff == '4'?
                   (function(){$("a.buyBtn").removeClass("ts_orange_btn").addClass("orange_btn");$("p.errorMsg").hide(); submitOrder();})()
	    ...
	    isAccountExist : function(){
          var url = url_config.isAccountExist,
              data =  {'format':'json','from':'pc'};
          sendAjaxRequest(url,data,'POST')
          .done(function(json){
            var code = response.responseCode;           
            if(code == '000000'){
                account.isAccountExist(function(responseData){
                  
                  if(responseData.isExist == '0' ){

                        window.location.href = url_config.infor+"&productId="+productId+'&amount='+money;             
            ...
            }else if (code == '903007' || code == '903008' || code =='903009') {//五项信息补全/证件类型不是身份证/主账户不存在
                //五项信息页面
                window.location.href = url_config.infor+"&productId="+productId+'&amount='+money;
            }else{
              $('a.buyBtn').addClass('canClick');
   ...
   */
	pageOperate = {
   	...
   	//未登陆流程封装
   	beforeloginedController = function(){
   		//判断是否输入购买额度，校验是否是数字
   	...
   	pageRenderController = function(){
   		//调用detail接口
   		var dtd = services(url_config,productId).productDetail(productId);
      if(dtd){
        render = new pageRender(detail_obj);
        $.when(//同步渲染
          $.Deferred(render.renderElm),//页面数据赋值
          $.Deferred(render.yuanObj),//返回数据处理
          $.Deferred(render.cuxiaoHTML),//活动规 则、促销文案等
          $.Deferred(render.titleRender),//页面数据赋值
          $.Deferred(render.amountRender),//页面数据赋值
          $.Deferred(render.timeLineRender),//计算时间轴
          $.Deferred(render.docRender),//相关文档赋值
          $.Deferred(render.btnRender),
          $.Deferred(render.eventBind)
        ).fail(function(){
           common.alert("系统繁忙，请稍后再试。");
        })
      } 
